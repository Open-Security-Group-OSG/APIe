...
def check(id_and_secret: str, list_id: int):
    ...
    try:
        if search(regex['censys'], id_and_secret):
            ...
            log.info(f'[bold cyan]ID:[/] [bold blue]{credentials[0]}[/] [bold cyan]SECRET:[/] [bold blue]{credentials[1]}[/] are [bold green]VALID[/] as [bold yellow]CENSYS CREDENTIALS[/] and have allowance of [bold cyan]{allowance}[/]')
            ...
            log.info(f'[bold cyan]ID:[/] [bold blue]{credentials[1]}[/] [bold cyan]SECRET:[/] [bold blue]{credentials[0]}[/] are [bold green]VALID[/]  as [bold yellow]CENSYS CREDENTIALS[/] and have allowance of [bold cyan]{allowance}[/]')
            censys_valid_keys.append([id_and_secret_reversed, allowance])
            return [id_and_secret_reversed, True, allowance]

        else:
            log.info(f'[bold blue]{id_and_secret}[/] are [bold red]INVALID[/] as [bold yellow]CENSYS CREDENTIALS[/] according to regex')
            invalid_keys[list_id].append(id_and_secret)
            return [id_and_secret, False]
    except:
        log.info(f'[bold blue]{id_and_secret}[/] are [bold red]INVALID[/] as [bold yellow]CENSYS CREDENTIALS[/] according to API call')
        invalid_keys[list_id].append(id_and_secret)
        return [id_and_secret, False]
